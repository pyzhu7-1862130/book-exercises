...
order(vehicles[vehicles$year == 1997, ], method = "hwy")
...
most_hwy <- function(year_choice, make_choice){
  year <- vehicles[vehicles$year == year_choice, ]
  make <- year[year$make == make_choice, ]
  return(make[make$hwy == max(make$hwy), "model"])
}
